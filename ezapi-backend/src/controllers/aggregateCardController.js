...
//get all systemAPIs of a user
const systemAPIController = async (req, res) => {
	try {
		const { user_id } = req;
...
//get all aggregateCards
const getAggregateCards = async (req, res) => {
	try {
		const { operationId, projectId } = req.query;
		const aggregationCards = await AggregateCards.find({ operationId, projectId }, {}).lean();
...
//get a single aggregateCard by id
const getAggregateCardById = async (req, res) => {
	try {
...
//create a new aggregateCard
const createAggregateCard = async (req, res) => {
	try {
		const {
			projectId,
			operationId,
...
//update an aggregateCard by id
const updateAggregateCard = async (req, res) => {
	try {
...
//delete an aggregateCard by id
const deleteAggregateCard = async (req, res) => {
	try {
		let { cardId } = req.params;
		const {
			projectId,
...
//get mapping data by the cardId
const getMappingDataByCardId = async (req, res) => {
	try {
		const { cardId } = req.params;
		...
		if (aggregateCard) return res.status(200).send({ aggregateApi, cardData: aggregateCard });
		throw new Error('Invalid OperationId or empty Data');
	} catch (err) {
		return res.status(400).send({ message: err.message });
	}
...
module.exports = {
	systemAPIController,
	getAggregateCards,
	getAggregateCardById,
	createAggregateCard,
	updateAggregateCard,
	deleteAggregateCard,
	getMappingDataByCardId
};
