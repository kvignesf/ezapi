...
// Download Spec
router.post('/download_spec', authenticate, async (req, res) => {
	try {
		...
		console.log("error in dwloadspec..", error);
		removeLocalStorgae(filepath);
		res.status(400).send({ error: error.message });
	}
});
router.post('/download_apigee', authenticate, async (req, res) => {
	try {
...
router.post('/download_gendata', authenticate, async (req, res) => {
	let projectid = req.body.projectId;
	...
	try {
		...
		try {
			...
			fs.writeFile(master_upd_filename, JSON.stringify(master_dump), (error) => {
				...
				exec(cmd, (error, stdout, stderr) => {
					if (error) {
						console.log(`error in dwnloadgendata: ${error.message}`);
						res.status(500).json({
							success: false,
		...
		console.log("error in dwnloadgendata..", error);
		res.status(400).send({ error: 'unexpected error' });
	}
});

// Download artefact
router.post('/download_apiops', authenticate, async (req, res) => {
	let projectid = req.body.projectId;
	let os_type = req.body.os_type ? req.body.os_type : 'others';
	let dbname = null;
	let offlinePath = 'offline/';
...
// Download codeGen
router.post('/download_codegen', authenticate, async (req, res) => {
	try {
		let projectid = req.body.projectId;
...
router.post('/download_dotnet_codegen', authenticate, async (req, res) => {
	try {
		let projectid = req.body.projectId;
		let project_data = await Projects.findOne({ projectId: projectid });
		let projectName = project_data['projectName'].replace('_', '').replace('-', '').replace('_', '');
...
// Download python codeGen 
router.post('/download_python_codegen', authenticate, async (req, res) => {
	try {
		let projectid = req.body.projectId;
		let project_data = await Projects.findOne({ projectId: projectid });
		...
		let codepath = '/mnt/codegen/' + projectid + "/pythoncode";
		if (platform().includes("win")) {
			codepath =  process.env.DIRECTORY_LOCATION+"/"+projectid+"/pythoncode";
		}
		let offlinePath = 'offline/';
		...
		const outpath = 'offline/' + projectid + '_python.zip';
		const outfile = fs.createWriteStream(outpath);
		const archive = archiver('zip');
		...
		const bucketFolder = 'proj_' + projectid;
		const filepath = outpath;
		const filename = 'proj_' + projectName + '_python.zip';
...
// Download node codeGen 
router.post('/download_node_codegen', authenticate, async (req, res) => {
	try {
		let projectid = req.body.projectId;
		let project_data = await Projects.findOne({ projectId: projectid });
		let projectName = project_data['projectName'].replace("_", "").replace("-","")
		
		let codepath = '/mnt/codegen/' + projectid + "/nodecode";
		if (platform().includes("win")) {
			codepath =  process.env.DIRECTORY_LOCATION+"/"+projectid+"/nodecode";
		}
		let offlinePath = 'offline/';
		...
		const outpath = 'offline/' + projectid + '_node.zip';
		const outfile = fs.createWriteStream(outpath);
		const archive = archiver('zip');
		...
		const bucketFolder = 'proj_' + projectid;
		const filepath = outpath;
		const filename = 'proj_' + projectName + '_node.zip';
