...
router.post(
	...
	async (req, res) => {
		try {
			...
			console.log("operData Request..", error);
...
router.post(
	...
	async (req, res) => {
		try {
			...
			console.log("sinkResponse..", error);
			res.status(400).send({ error: error.message });
		}
	}
);
...
router.post(
	...
	async (req, res) => {
		try {
			...
			console.log("operData Response..", error);
			res.status(400).send({ error: error.message });
		}
	}
);

router.post('/simulation_artefacts', authenticate, async (req, res) => {
	try {
		...
		request.post(options, async function (err, httpResponse, body) {
			if (err) {
				return { message: 'Unable to generate Simulation Data' };
			}
			const { statusCode } = httpResponse;
			if (statusCode == 500) {
				res.status(500).send({ message: httpResponse.body.message });
			} else {
				return res.status(200).json({ message: 'Ok' });
			}
			//if (!err) res.status(httpResponse.statusCode).json({ response: httpResponse });
			//else res.status(500).json({ err: err.message , message: err.message.message});
		});
	} catch (err) {
...
async function addObject(item) {
	...
	if (objectType === 'attribute') {
		itemBody = {
			...
			name: item.name,
			id: item.id,
			is_child: !item.is_child ? false : true,
			type: item.type,
	...
	} else if (objectType === 'ezapi_table' || objectType === 'documentField') {
		itemBody = {
			payloadId: item.payloadId,
			is_child: !item.is_child ? false : true,
...
async function addObjectToArray(object) {
	...
	var mapItem = {
		...
		name: object.name,
		type: object.type,
		is_child: !object.is_child ? false : true,
	...
	if (object.paramType) {
		mapItem.is_child = !object.is_child ? false : true;
