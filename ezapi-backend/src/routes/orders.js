...
router.get('/products', authenticate, async (req, res) => {
	try {
		...
		if (
			!grpUsr.group ||
			grpUsr.group == null ||
			grpUsr.group == undefined ||
			grpUsr.group.length == 0
		) {
			grpUsr.group = 'regular';
		}
		const planBenefitsList = await Products.find({
			isDisabled: false,
			group: grpUsr.group
		}).lean();
		planBenefitsList.forEach((planBenefit) => {
			...
			//assign unlimited for projects with 999 or more limit
			const unlimitedProjs = Object.keys(planBenefit).filter(
				(key) => planBenefit[key] >= 999
			);
			for (var i of unlimitedProjs) {
				planBenefit[i] = 'unlimited';
			}
		});
...
router.get('/orders', authenticate, async (req, res) => {
	try {
		...
		if (user) {
			...
			//console.log("stripCancellations", stripe.can)
...
router.post('/billing-details', authenticate, async (req, res) => {
	try {
...
router.post('/subscribe', authenticate, async (req, res) => {
	try {
		...
		if (!price_id) {
			throw new Error('Price_id is a required field');
		}
		const { user_id } = req;
		const user = await Users.findOne({ user_id });
		if (user) {
...
router.post('/makeDefaultCard', authenticate, async (req, res) => {
	try {
		let { user_id } = req;
		let { cardId } = req.body;
		const user = await Users.findOne({ user_id });
...
router.get('/listCards', authenticate, async (req, res) => {
	try {
		let { user_id } = req;
		const user = await Users.findOne({ user_id });
		if (user) {
...
router.post('/cancelSubscription', authenticate, async (req, res) => {
	try {
		let { user_id } = req;
		const user = await Users.findOne({ user_id });
...
router.post('/initiate-order', authenticate, async (req, res) => {
	try {
	...
	try {
		...
		switch (event.type) {
			...
			case 'customer.subscription.updated':
				...
				if (user) {
					if (intent.cancel_at_period_end) {
						...
						//updatePublishLimit(user);
						if (product) {
							if (interval === 'month') {
								//user.allowedProjects = product.no_of_projects;
								user.designProjects = 999;
								user.testProjects = 999;
								user.noSpecNoDb = 999;
								user.aggregateProjects = 999;
							}
							if (interval === 'year') {
								//user.allowedProjects = product.no_of_projects * 12;
								user.designProjects = 9999;
								user.testProjects = 9999;
								user.noSpecNoDb = 9999;
								user.aggregateProjects = 9999;
							}
						}
			...
			case 'charge.failed':
				intent = event.data.object;
				user = await Users.findOne({ stripeCustomerId: intent.customer });
				product = await Products.findOne({ stripe_product_id: { $exists: false } });
				if (user) {
