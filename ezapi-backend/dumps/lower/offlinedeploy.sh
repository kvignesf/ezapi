...
mongoTagsArr=$(curl -H "PRIVATE-TOKEN: bMJKJVzze8Dzwcf2r9s3" "https://gitlab.com/api/v4/registry/repositories/1454657?tags=true&tags_count=true&size=true" | sed -e 's/[{}]/''/g' | awk '{print $2}'  FS='tags":' | awk -F'[][]' '{print $2}' | grep -oP '(?<=\"name\"\:).*?(?=,)')
echo 'Please select from the tag options for MongoDB:'
SelectionFromUser $mongoTagsArr
count=$?
n=""
while true; do
...
mongoTagVal=$(readElementfrmArr $n $mongoTagsArr);
mongoTagVal=`sed -e 's/^"//' -e 's/"$//' <<<"$mongoTagVal"`
echo "You selected option number $n: $mongoTagVal"

nodeTagsArr=$(curl -H "PRIVATE-TOKEN: bMJKJVzze8Dzwcf2r9s3" "https://gitlab.com/api/v4/registry/repositories/1456307?tags=true&tags_count=true&size=true" | sed -e 's/[{}]/''/g' | awk '{print $2}'  FS='tags":' | awk -F'[][]' '{print $2}' | grep -oP '(?<=\"name\"\:).*?(?=,)')
echo 'Please select from the tag options for Node Repo:'
SelectionFromUser $nodeTagsArr
count=$?
n=""
while true; do
    read -p 'Select option: ' n
...
nodeTagVal=$(readElementfrmArr $n $nodeTagsArr);
nodeTagVal=`sed -e 's/^"//' -e 's/"$//' <<<"$nodeTagVal"`
echo "You selected option number $n: $nodeTagVal"

reactTagsArr=$(curl -H "PRIVATE-TOKEN: bMJKJVzze8Dzwcf2r9s3" "https://gitlab.com/api/v4/registry/repositories/1454591?tags=true&tags_count=true&size=true" | sed -e 's/[{}]/''/g' | awk '{print $2}'  FS='tags":' | awk -F'[][]' '{print $2}' | grep -oP '(?<=\"name\"\:).*?(?=,)')
echo 'Please select from the tag options for React Repo:'
SelectionFromUser $reactTagsArr
count=$?
n=""
while true; do
    read -p 'Select option: ' n
...
reactTagVal=$(readElementfrmArr $n $reactTagsArr);
reactTagVal=`sed -e 's/^"//' -e 's/"$//' <<<"$reactTagVal"`
echo "You selected option number $n: $reactTagVal"

testRunnerTagsArr=$(curl -H "PRIVATE-TOKEN: bMJKJVzze8Dzwcf2r9s3" "https://gitlab.com/api/v4/registry/repositories/1596924?tags=true&tags_count=true&size=true" | sed -e 's/[{}]/''/g' | awk '{print $2}'  FS='tags":' | awk -F'[][]' '{print $2}' | grep -oP '(?<=\"name\"\:).*?(?=,)')
echo 'Please select from the tag options for TestRunner Repo:'
SelectionFromUser $testRunnerTagsArr
count=$?
n=""
while true; do
    read -p 'Select option: ' n
...
testRunnerTagVal=$(readElementfrmArr $n $testRunnerTagsArr);
testRunnerTagVal=`sed -e 's/^"//' -e 's/"$//' <<<"$testRunnerTagVal"`
echo "You selected option number $n: $testRunnerTagVal"

virtualServTagsArr=$(curl -H "PRIVATE-TOKEN: bMJKJVzze8Dzwcf2r9s3" "https://gitlab.com/api/v4/registry/repositories/1484630?tags=true&tags_count=true&size=true" | sed -e 's/[{}]/''/g' | awk '{print $2}'  FS='tags":' | awk -F'[][]' '{print $2}' | grep -oP '(?<=\"name\"\:).*?(?=,)')
echo 'Please select from the tag options for VirtualService Repo:'
SelectionFromUser $virtualServTagsArr
count=$?
n=""
while true; do
    read -p 'Select option: ' n
...
virtualServTagVal=$(readElementfrmArr $n $virtualServTagsArr);
virtualServTagVal=`sed -e 's/^"//' -e 's/"$//' <<<"$virtualServTagVal"`
echo "You selected option number $n: $virtualServTagVal"


dataEnablerTagsArr=$(curl -H "PRIVATE-TOKEN: bMJKJVzze8Dzwcf2r9s3" "https://gitlab.com/api/v4/registry/repositories/2992124?tags=true&tags_count=true&size=true" | sed -e 's/[{}]/''/g' | awk '{print $2}'  FS='tags":' | awk -F'[][]' '{print $2}' | grep -oP '(?<=\"name\"\:).*?(?=,)')
echo 'Please select from the tag options for dataEnablerTagVal:'
SelectionFromUser $dataEnablerTagsArr
count=$?
n=""
while true; do
    read -p 'Select option: ' n
...
dataEnablerTagVal=$(readElementfrmArr $n $dataEnablerTagsArr);
